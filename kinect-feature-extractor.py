...
class BodyGameRuntime(object):
    ...
    def print_skeleton_data(self, joints, timer):
        ...
        for jointType in jointTypes:
            ...
            normPosX = joints[jointType].Position.x - joints[PyKinectV2.JointType_SpineMid].Position.x;
            normPosY = joints[jointType].Position.y - joints[PyKinectV2.JointType_SpineMid].Position.y;
            normPosZ = joints[jointType].Position.z - joints[PyKinectV2.JointType_SpineMid].Position.z;
        ...
        #for i, pos in enumerate(positionLst):
            #velocity = round(abs(float(1000*(pos - positionBuffer[i])/timer)), 4) #velocity in /s
            #velocityLst.append(velocity)

        print(positionLst);
        
        for i, listitem in enumerate(positionLst):
            f.write('%s' % listitem)
            if i is not (len(positionLst) - 1):
                f.write(',')
        f.write('\n')
        ...
        while not self._done:
                ...
                if event.type == pygame.QUIT: # If user clicked close
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        if recording:
                            ...
                            f = open(filename,'a+')
                            positionBuffer = [] #empties the positionbuffer before recording
            ...
            if self._bodies is not None: 
                for i in range(0, self._kinect.max_body_count):
                    ...
                    if recording and timer > 150:
                        self.print_skeleton_data(joints, timer)
                        timer = 0
                    ...
                    #print(joint_points[PyKinectV2.JointType_Head])
